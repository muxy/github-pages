...
description: Archived Muxy documentation for Trivia Contests.
slug: trivia
product: MEDKit
audience: developers
status: archived
...
```javascript title="Setting and updating questions"
...
```javascript title="Admin sets a question with two options"
...
![1024](../assets/readme/3af5e52-game_awards.png){ width="1024" height="768" loading="lazy" }
...
```javascript title="Amin updates question text"
const opts = new Muxy.DebuggingOptions();
opts.role('admin');
Muxy.debug(opts);
...
```javascript title="Admin removes a question"
// remove an option
...
```javascript title="Setting questions"
...
```javascript title="Admin sets a question"
const opts = new Muxy.DebuggingOptions();
opts.role('admin');
Muxy.debug(opts);
...
```javascript title="Admin declares winning option"
const opts = new Muxy.DebuggingOptions();
opts.role('admin');
Muxy.debug(opts);
...
```javascript title="Retrieve question in Viewer extension"
const opts = new Muxy.DebuggingOptions();
opts.role('viewer');
Muxy.debug(opts);
...
```javascript title="Send viewer vote"
const opts = new Muxy.DebuggingOptions();
opts.role('viewer');
Muxy.debug(opts);
...
```javascript title="Request leaderboard status"
const opts = new Muxy.DebuggingOptions();
opts.role('admin');
Muxy.debug(opts);
